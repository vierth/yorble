...
def align(quote1, quote2,matchscore,misalignscore,mismatchscore, chunklim, overlap, rangematch):
...
# Run the process
def runalignment(content,totallength,matchscore,misalignscore,mismatchscore, chunklim, overlap, rangematch):
    global tracker
    info = content.split("\t")
    # If the quotes are identical, no need to align them
    if float(info[3]) != 1.0:
        # Run the alignment algorithm
        aligneda, alignedb = align(info[6], info[7],matchscore,misalignscore,mismatchscore, chunklim, overlap, rangematch)
...
# START OF MAIN LOGIC #
#*********************#
def run(alignment_docs,matchscore, misalignscore, mismatchscore, chunklim, overlap,rangematch, corpusresults, outputfile):
    ...
    with open(corpusresults, "r") as rf:
        ...
        results = pool.starmap(runalignment,zip(contents,repeat(len(contents)),repeat(matchscore),repeat(misalignscore),repeat(mismatchscore), repeat(chunklim), repeat(overlap), repeat(rangematch)))
...
if __name__ == "__main__":
    ...
    run(alignment_docs,matchscore, misalignscore, mismatchscore, chunklim, overlap,rangematch, corpusresults, outputfile)